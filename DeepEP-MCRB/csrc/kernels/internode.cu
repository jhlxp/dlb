...
__device__ __forceinline__ uint64_t load_nvl_rank_mask_as_bool_bytes(const bool* is_token_in_rank,
                                                                     int64_t token_idx,
                                                                     int num_ranks,
                                                                     int dst_rdma_rank) {
    uint64_t mask = 0;
    ...
    for (int i = 0; i < NUM_MAX_NVL_PEERS; ++ i) {
        auto value = __ldg(reinterpret_cast<const unsigned char*>(
            is_token_in_rank + token_idx * num_ranks + dst_rdma_rank * NUM_MAX_NVL_PEERS + i));
        mask |= static_cast<uint64_t>(value) << (i * sizeof(bool) * 8);
    }
    return mask;
}
    ...
    if (sm_id == 0) {
        ...
        for (int channel_id = warp_id; channel_id < num_channels; channel_id += num_warps) {
            ...
            for (int64_t i = token_start_idx + lane_id; i < token_end_idx; i += 32) {
                auto is_token_in_rank_uint64 = load_nvl_rank_mask_as_bool_bytes(is_token_in_rank, i, num_ranks, dst_rdma_rank);
    ...
    if (warp_role == WarpRole::kRDMASender) {
        ...
        for (token_idx = token_start_idx; token_idx < token_end_idx; ++ token_idx) {
            ...
            if (lane_id < kNumRDMARanks) {
                is_token_in_rank_uint64 = load_nvl_rank_mask_as_bool_bytes(is_token_in_rank, token_idx, num_ranks, lane_id);
