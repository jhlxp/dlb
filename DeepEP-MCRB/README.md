...
This directory is a minimal DeepEP V1 fork for Mask-Constrained Rail Balancing
(MCRB). The baseline DeepEP data path is kept intact by default. MCRB is being
added as an embedded Rail selector for 2x4 logical-server experiments: each
token message may use either its original source Rail or one real destination
Rail from its destination mask, avoiding unrelated third-party Rails.

The clean upstream-like baseline is `../deepepv1-raw`; the older full-DLB
prototype is `../DeepEP-LB` and should be treated as reference code only.

For 2x4 logical-server experiments on one 8-GPU node, build with:

```bash
NUM_MAX_NVL_PEERS=4 TORCH_CUDA_ARCH_LIST=9.0 NVSHMEM_DIR=/path/to/nvshmem python3 setup.py build
```

Without `NUM_MAX_NVL_PEERS=4`, the raw V1 layout treats 8 ranks as one NVLink
domain instead of two logical servers with four Rails each.
