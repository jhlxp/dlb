...
DeepEP-LB keeps the DeepEP V1 data plane and adds optional source-server-local
Rail load balancing for high-throughput internode MoE dispatch. The default
`use_lb=False` path retains the original DeepEP V1 behavior.

DLB changes only the physical source Rail. Token-to-Rank deduplication,
dispatch/combine, destination-side NVLink forwarding, NVSHMEM/IBGDA
transport, BF16/FP8 support, and asynchronous execution remain in the DeepEP
V1 runtime. This branch targets H200 and compiles only for SM90.

There is no standalone per-token planner or second pack:

1. The existing notify kernel gathers one demand row from each source Rail
   over CUDA IPC and computes a deterministic compact quota table. Its
   per-destination block uses one block-wide token scan to produce flow
   ordinals, channel prefixes, and selected-Rail counts together.
2. The existing persistent dispatch kernel selects a Rail while scanning and
   packing tokens.
3. Tokens within the source Rail's quota keep the original V1 path.
4. Only surplus tokens make one extra NVLink hop into the selected Rail's
   staging area before entering the original V1 RDMA ring.
5. The existing persistent combine kernel returns only moved partial rows to
   the origin Rail and folds them into the direct result. Its exact-size
   handle stores only the compact schedule and moved-token reverse mapping;
   unused workspace, ready flags, and payload data are not copied.

Enable DLB when constructing the existing Buffer:
...
buffer = deep_ep.Buffer(
    ...
    num_rdma_bytes,
    use_lb=True,
    lb_max_tokens=max_local_tokens,
    lb_hidden=hidden,
    lb_num_topk=num_topk,
)
```

`lb_max_tokens`, `lb_hidden`, and `lb_num_topk` size the preallocated surplus
token staging area. The data plane performs no dynamic allocation. DLB is
available only for high-throughput internode mode. Production transport
remains NVSHMEM/IBGDA. The CUDA IPC/P2P implementation under `tests/` models
source staging and moved-token reverse repair around an omitted network
boundary in environments without an accessible HCA; it is not an RDMA
performance measurement.

The build-time `DEEPEP_NUM_NVL_PEERS` value is the number of GPUs/Rails in
each physical server. Set it to `4` for a four-GPU server and `8` for an
eight-GPU server. It is a topology parameter, not a test-only GPU mapping.
...
## Upstream DeepEP V1 reference performance

The following tables are inherited from upstream DeepEP V1. They describe the
unmodified V1 data plane on its original hardware and are not DeepEP-LB
benchmark claims.
...
```bash
# Build the H200/SM90 extension for a four-GPU-per-server topology.
DEEPEP_NUM_NVL_PEERS=4 \
NVSHMEM_DIR=/path/to/installed/nvshmem \
python setup.py build_ext --inplace

# Run the original V1 EP8 intranode regression with an eight-GPU NVLink
# domain. This is a separate build from the four-GPU-per-server example.
DEEPEP_NUM_NVL_PEERS=8 \
NVSHMEM_DIR=/path/to/installed/nvshmem \
python setup.py build_ext --inplace
...
# These require an accessible HCA and the production topology build.
python tests/test_internode.py
python tests/test_low_latency.py

# Validate compact deterministic quotas and exact-size moved-only handles.
CUDA_VISIBLE_DEVICES=0 python tests/test_dlb_compact_quota.py
CUDA_VISIBLE_DEVICES=0 python tests/test_dlb_handle.py
CUDA_VISIBLE_DEVICES=0 python tests/test_dlb_distributions.py

# Run the test-only EP8, 2-logical-server CUDA IPC/P2P data plane.
# The directed 5000/6000/7000/8000 case must become 6500 on every Rail.
CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 \
python tests/test_dlb_loopback.py \
  --hidden 2048 \
  --topk 8 \
  --dtype both
```
...
- `DEEPEP_NUM_NVL_PEERS`: GPUs/Rails per physical server; supported values are `4` and `8`
- `TORCH_CUDA_ARCH_LIST`: forced to `9.0` by this fork
- `DISABLE_SM90_FEATURES`: unsupported; a nonzero value is rejected
