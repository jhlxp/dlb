...
using NvlRankMask = typename VecInt<NUM_MAX_NVL_PEERS * sizeof(bool)>::vec_t;

struct SourceMeta {
    int src_rdma_rank, is_token_in_nvl_rank_bits;

    EP_STATIC_ASSERT(NUM_MAX_NVL_PEERS <= sizeof(is_token_in_nvl_rank_bits) * 8,
                     "Too many NVLink peers for the destination mask");
...
template <bool kLowLatencyMode, int kNumRDMARanks, bool kLoadBalance>
__global__ void
notify_dispatch(const int* num_tokens_per_rank, int* moe_recv_counter_mapped, int num_ranks,
                ...
                int64_t lb_workspace_offset,
                int lb_max_tokens, size_t lb_record_stride,
                int lb_round_id,
                int* lb_virtual_count, int* lb_outgoing_count,
    ...
    auto lb_workspace_for = [&](int source_nvl_rank) {
        auto base = static_cast<uint8_t*>(buffer_ptrs[source_nvl_rank]) +
                    lb_workspace_offset;
        return rail_lb::Workspace(
            base, lb_max_tokens, kNumRDMARanks, num_channels,
            lb_record_stride
        );
    };
    constexpr int kLBScanWarps = 16;
    constexpr int kLBScanEntries = kLBScanWarps + 1;
    constexpr int kLBCountEntries =
        rail_lb::kMaxChannels * NUM_MAX_NVL_PEERS *
        (NUM_MAX_NVL_PEERS + 1);
    constexpr int kLBSharedEntries =
        kLoadBalance
            ? kLBScanEntries + kLBCountEntries
            : 1;
    __shared__ int lb_shared[kLBSharedEntries];
    auto lb_channel_counts =
        lb_shared + (kLoadBalance ? kLBScanEntries : 0);

    if constexpr (kLoadBalance) {
        if (sm_id == 0) {
            const int epoch = lb_round_id + 1;
            auto local_workspace = lb_workspace_for(nvl_rank);
            for (int server = thread_id;
                 server < kNumRDMARanks;
                 server += num_threads) {
                local_workspace.source_loads[server] =
                    num_tokens_per_rdma_rank[server];
                local_workspace.route_ready[server] = 0;
            }
            for (int token = thread_id;
                 token < num_tokens;
                 token += num_threads) {
                local_workspace.repair_heads[token] = -1;
            }
            if (thread_id == 0) {
                *local_workspace.num_tokens = num_tokens;
                *local_workspace.num_outgoing = 0;
                *local_workspace.epoch = epoch;
                memory_fence();
                st_release_sys_global(
                    local_workspace.demand_ready, epoch
                );
            }
            __syncthreads();
            ...
            if (thread_id == 0) {
                #pragma unroll
                for (int rail = 0;
                     rail < NUM_MAX_NVL_PEERS;
                     ++rail) {
                    auto peer_workspace = lb_workspace_for(rail);
                    while (ld_acquire_sys_global(
                               peer_workspace.demand_ready
                           ) != epoch) {
                        __nanosleep(NUM_WAIT_NANOSECONDS);
                    }
                }

                for (int destination_server = 0;
                     destination_server < kNumRDMARanks;
                     ++destination_server) {
                    int loads[NUM_MAX_NVL_PEERS];
                    #pragma unroll
                    for (int rail = 0;
                         rail < NUM_MAX_NVL_PEERS;
                         ++rail) {
                        loads[rail] =
                            lb_workspace_for(rail).source_loads[
                                destination_server
                            ];
                    }
                    rail_lb::compute_quota_row(
                        loads,
                        rdma_rank,
                        destination_server,
                        nvl_rank,
                        lb_round_id,
                        local_workspace.quota_row(destination_server)
                    );
                }
                memory_fence();
                st_release_sys_global(
                    local_workspace.quota_ready, epoch
                );
            }
            __syncthreads();
        }
    ...
    if (sm_id == 0) {
        ...
        if constexpr (kLoadBalance) {
            auto local_workspace = lb_workspace_for(nvl_rank);
            const int epoch = *local_workspace.epoch;
            if (thread_id == 0) {
                // Metadata blocks on every local GPU publish selected-Rail
                // channel counts before the notify communication uses them.
                #pragma unroll
                for (int producer = 0;
                     producer < NUM_MAX_NVL_PEERS;
                     ++producer) {
                    auto producer_workspace = lb_workspace_for(producer);
                    for (int server = 0; server < kNumRDMARanks; ++server) {
                        while (ld_acquire_sys_global(
                                   producer_workspace.route_ready + server
                               ) != epoch) {
                            __nanosleep(NUM_WAIT_NANOSECONDS);
                        }
                    }
                ...
                // Each producer writes only its own compact offsets. Stage
                // slots are laid out by selected Rail, producer, server and
                // channel; repair slots are local and contain moved routes
                // only.
                int repair_cursor = 0;
                #pragma unroll
                for (int selected = 0;
                     ...
                     ++selected) {
                    int incoming_cursor = 0;
                    #pragma unroll
                    for (int producer = 0;
                         producer < NUM_MAX_NVL_PEERS;
                         ++producer) {
                        auto producer_workspace =
                            lb_workspace_for(producer);
                        for (int server = 0;
                             server < kNumRDMARanks;
                             ++server) {
                            for (int channel = 0;
                                 ...
                                 ++channel) {
                                const int index =
                                    producer_workspace.route_index(
                                        selected, server, channel,
                                        kNumRDMARanks, num_channels
                                    );
                                if (producer == nvl_rank)
                                    local_workspace.stage_offsets[index] =
                                        incoming_cursor;
                    ...
                    if (selected != nvl_rank) {
                        for (int server = 0;
                            ...
                            for (int channel = 0;
                                 ...
                                 ++channel) {
                                const int index =
                                    local_workspace.route_index(
                                        selected, server, channel,
                                        kNumRDMARanks, num_channels
                                    );
                                local_workspace.repair_offsets[index] =
                                    repair_cursor;
                                const int count =
                                    local_workspace.route_counts[index];
                                repair_cursor += count;
                                outgoing += count;
                            }
                        }
                    ...
                    if (selected == nvl_rank) {
                        EP_DEVICE_ASSERT(
                            static_cast<size_t>(incoming_cursor) <=
                            local_workspace.payload_capacity
                        );
                        *lb_virtual_count =
                            num_tokens + incoming_cursor;
                    }
                }
                EP_DEVICE_ASSERT(
                    static_cast<size_t>(repair_cursor) <=
                    local_workspace.payload_capacity
                );
                *local_workspace.num_outgoing = repair_cursor;
                *lb_outgoing_count = repair_cursor;
                memory_fence();
                st_release_sys_global(
                    local_workspace.schedule_ready, epoch
                );
            }
            __syncthreads();
        }
        ...
        for (int i = thread_id; i < num_ranks; i += num_threads) {
            int count = num_tokens_per_rank[i];
            if constexpr (kLoadBalance) {
                const int destination_server = i / NUM_MAX_NVL_PEERS;
                const int destination_nvl_rank = i % NUM_MAX_NVL_PEERS;
                count = 0;
                #pragma unroll
                for (int source_nvl_rank = 0;
                     source_nvl_rank < NUM_MAX_NVL_PEERS;
                     ++source_nvl_rank) {
                    auto source_workspace = lb_workspace_for(source_nvl_rank);
                    for (int channel = 0; channel < num_channels; ++channel) {
                        count += source_workspace.mask_counts[
                            source_workspace.mask_index(
                                nvl_rank, destination_server, channel,
                                destination_nvl_rank,
                                kNumRDMARanks, num_channels
                            )
                        ];
                    }
        ...
        if (thread_id < kNumRDMARanks) {
            int count = num_tokens_per_rdma_rank[thread_id];
            if constexpr (kLoadBalance) {
                count = 0;
                #pragma unroll
                for (int source_nvl_rank = 0;
                    ...
                    auto source_workspace = lb_workspace_for(source_nvl_rank);
                    for (int channel = 0; channel < num_channels; ++channel) {
                        count += source_workspace.route_counts[
                            source_workspace.route_index(
                                nvl_rank, thread_id, channel,
                                kNumRDMARanks, num_channels
                            )
                        ];
                    }
        ...
        // Calculate metadata for one destination server. The LB path performs
        // one block-wide scan that simultaneously establishes flow ordinals,
        // channel prefixes, and physical Rail counts. No per-token plan is
        // materialized.
        const int dst_rdma_rank = sm_id - 1;
        auto local_workspace = lb_workspace_for(nvl_rank);

        if constexpr (kLoadBalance) {
            EP_DEVICE_ASSERT(num_warps == kLBScanWarps);
            const int epoch = *local_workspace.epoch;
            while (ld_acquire_sys_global(local_workspace.quota_ready) !=
                   epoch) {
                __nanosleep(NUM_WAIT_NANOSECONDS);
            }

            const int per_channel_entries =
                NUM_MAX_NVL_PEERS * (NUM_MAX_NVL_PEERS + 1);
            for (int index = thread_id;
                 index < num_channels * per_channel_entries;
                 index += num_threads) {
                lb_channel_counts[index] = 0;
            }
            __syncthreads();

            // One pass: block-wide valid-token prefix plus Rail/mask counts.
            auto lb_warp_offsets = lb_shared;
            auto lb_chunk_count = lb_shared + kLBScanWarps;
            int flow_prefix = 0;
            for (int channel_id = 0;
                ...
                get_channel_task_range(
                    num_tokens, num_channels, channel_id,
                    token_start_idx, token_end_idx
                );
                int channel_count = 0;
                for (int token_base = token_start_idx;
                     token_base < token_end_idx;
                     token_base += num_threads) {
                    const int token = token_base + thread_id;
                    NvlRankMask rank_mask = 0;
                    if (token < token_end_idx) {
                        rank_mask =
                            *reinterpret_cast<const NvlRankMask*>(
                                is_token_in_rank +
                                token * num_ranks +
                    ...
                    const bool valid =
                        token < token_end_idx and rank_mask != 0;
                    const auto valid_lanes =
                        __ballot_sync(0xffffffffu, valid);
                    const int warp_count = __popc(valid_lanes);
                    const int warp_ordinal = __popc(
                        valid_lanes & ((1u << lane_id) - 1u)
                    );
                    if (lane_id == 0)
                        lb_warp_offsets[warp_id] = warp_count;
                    __syncthreads();
                    if (thread_id == 0) {
                        int chunk_count = 0;
                        #pragma unroll
                        for (int warp = 0;
                             ...
                             ++warp) {
                            const int count =
                                lb_warp_offsets[warp];
                            lb_warp_offsets[warp] = chunk_count;
                            chunk_count += count;
                        }
                        *lb_chunk_count = chunk_count;
                    }
                    __syncthreads();

                    int selected = rail_lb::kInvalidRail;
                    if (valid) {
                        const int ordinal =
                            flow_prefix + channel_count +
                            lb_warp_offsets[warp_id] + warp_ordinal;
                        selected = rail_lb::select_rail(
                            local_workspace.quota_row(dst_rdma_rank),
                            nvl_rank,
                            ordinal
                        );
                        EP_DEVICE_ASSERT(
                        ...
                        atomicAdd(
                            lb_channel_counts +
                                channel_id * per_channel_entries +
                                selected,
                            1
                        );
                        const auto mask_values =
                            reinterpret_cast<const bool*>(&rank_mask);
                        #pragma unroll
                        for (int destination_rail = 0;
                             destination_rail < NUM_MAX_NVL_PEERS;
                             ++destination_rail) {
                            if (mask_values[destination_rail]) {
                                atomicAdd(
                                    lb_channel_counts +
                                        channel_id *
                                            per_channel_entries +
                    ...
                    const int chunk_count = *lb_chunk_count;
                    __syncthreads();
                    channel_count += chunk_count;
                }
                flow_prefix += channel_count;
                if (thread_id == 0) {
                    local_workspace.source_channel_prefix[
                        local_workspace.source_channel_index(
                            dst_rdma_rank, channel_id, num_channels
                        )
                    ] = flow_prefix;
                }
                __syncthreads();
            ...
            for (int index = thread_id;
                 index < num_channels * per_channel_entries;
                 index += num_threads) {
                const int channel = index / per_channel_entries;
                const int local_index = index % per_channel_entries;
                if (local_index < NUM_MAX_NVL_PEERS) {
                    const int selected = local_index;
                    local_workspace.route_counts[
                        local_workspace.route_index(
                            selected, dst_rdma_rank, channel,
                            kNumRDMARanks, num_channels
                        )
                    ] = lb_channel_counts[index];
                } else {
                    const int mask_index =
                        local_index - NUM_MAX_NVL_PEERS;
                    const int selected =
                        mask_index / NUM_MAX_NVL_PEERS;
                    const int destination_rail =
                        mask_index % NUM_MAX_NVL_PEERS;
                    local_workspace.mask_counts[
                        local_workspace.mask_index(
                            selected, dst_rdma_rank, channel,
                            destination_rail,
                            kNumRDMARanks, num_channels
                        )
            ...
            if (thread_id == 0) {
                memory_fence();
                st_release_sys_global(
                    local_workspace.route_ready + dst_rdma_rank,
                    epoch
                );
            }
            __syncthreads();
            ...
            for (int producer = 0;
                ...
                auto producer_workspace = lb_workspace_for(producer);
                while (ld_acquire_sys_global(
                           producer_workspace.route_ready + dst_rdma_rank
                       ) != epoch) {
                    __nanosleep(NUM_WAIT_NANOSECONDS);
                }
            }
        ...
        for (int channel_id = warp_id;
            ...
            if constexpr (kLoadBalance) {
                if (lane_id == 0) {
                    ...
                    for (int producer = 0;
                        ...
                        auto producer_workspace =
                            lb_workspace_for(producer);
                        total_count +=
                            producer_workspace.route_counts[
                                producer_workspace.route_index(
                                    nvl_rank, dst_rdma_rank, channel_id,
                                    kNumRDMARanks, num_channels
                                )
                            ];
                        #pragma unroll
                        for (int destination_rail = 0;
                             destination_rail < NUM_MAX_NVL_PEERS;
                             ++destination_rail) {
                            per_nvl_rank_count[destination_rail] +=
                                producer_workspace.mask_counts[
                                    producer_workspace.mask_index(
                                        nvl_rank, dst_rdma_rank,
                                        channel_id, destination_rail,
                                        kNumRDMARanks, num_channels
                                    )
                                ];
                        }
                ...
                for (int token = token_start_idx + lane_id;
                     token < token_end_idx;
                     token += 32) {
                    auto rank_mask =
                        *reinterpret_cast<const NvlRankMask*>(
                            is_token_in_rank + token * num_ranks +
                            dst_rdma_rank * NUM_MAX_NVL_PEERS
                        );
                    auto values =
                        reinterpret_cast<const bool*>(&rank_mask);
                    ...
                    for (int rail = 0;
                         rail < NUM_MAX_NVL_PEERS;
                         ++rail) {
                        per_nvl_rank_count[rail] += values[rail];
                    }
                    total_count += rank_mask != 0;
                }
                ...
                for (int rail = 0;
                     rail < NUM_MAX_NVL_PEERS;
                     ++rail) {
                    per_nvl_rank_count[rail] =
                        warp_reduce_sum(per_nvl_rank_count[rail]);
                }
            }
            ...
            if (lane_id == 0) {
                ...
                for (int rail = 0;
                    ...
                    gbl_channel_prefix_matrix[
                        (dst_rdma_rank * NUM_MAX_NVL_PEERS + rail) *
                            num_channels +
                        channel_id
                    ] = per_nvl_rank_count[rail];
                }
                rdma_channel_prefix_matrix[
                    dst_rdma_rank * num_channels + channel_id
                ] = total_count;
            }
        }
        ...
        if (thread_id == 0) {
            auto prefix_row =
                rdma_channel_prefix_matrix +
                dst_rdma_rank * num_channels;
            for (int channel = 1;
                 channel < num_channels;
                 ++channel) {
                prefix_row[channel] += prefix_row[channel - 1];
            }
        }
        if (thread_id < NUM_MAX_NVL_PEERS) {
            auto prefix_row =
...
void notify_dispatch(const int* num_tokens_per_rank, int* moe_recv_counter_mapped, int num_ranks,
                     ...
                     bool load_balance, int64_t lb_workspace_offset,
                     int lb_max_tokens, size_t lb_record_stride,
                     int lb_round_id,
                     int* lb_virtual_count, int* lb_outgoing_count,
                     int* rdma_channel_prefix_matrix, int* recv_rdma_rank_prefix_sum,
...
#define NOTIFY_DISPATCH_LAUNCH_CASE(num_rdma_ranks) { \
    auto notify_dispatch_func = low_latency_mode ? \
        (load_balance ? notify_dispatch<true, num_rdma_ranks, true> : \
                        notify_dispatch<true, num_rdma_ranks, false>) : \
        (load_balance ? notify_dispatch<false, num_rdma_ranks, true> : \
                        notify_dispatch<false, num_rdma_ranks, false>); \
    LAUNCH_KERNEL(&cfg, notify_dispatch_func, \
                  ...
                  lb_workspace_offset, lb_max_tokens, lb_record_stride, lb_round_id, \
                  lb_virtual_count, lb_outgoing_count, \
...
template <bool kLowLatencyMode, int kNumRDMARanks, bool kCachedMode,
          bool kLoadBalance, int kNumTMABytesPerWarp,
...
dispatch(int4* recv_x, float* recv_x_scales, int64_t* recv_topk_idx, float* recv_topk_weights, SourceMeta* recv_src_meta,
         ...
         int64_t lb_workspace_offset, int lb_max_tokens,
         size_t lb_record_stride,
    ...
    EP_STATIC_ASSERT(NUM_MAX_NVL_PEERS * sizeof(bool) == sizeof(NvlRankMask), "Invalid number of NVL peers");
    ...
    auto lb_workspace_for = [&](int local_rank) {
        auto base = static_cast<uint8_t*>(buffer_ptrs[local_rank]) +
                    lb_workspace_offset;
        return rail_lb::Workspace(
            base, lb_max_tokens, kNumRDMARanks,
            num_channels, lb_record_stride
        );
    };
    ...
    if (warp_role == WarpRole::kRDMASender) {
        ...
        // Keep direct messages on the original V1 packing path. In DLB mode,
        // each lane masks out only the routes moved to another source Rail.
        int64_t token_idx;
        int cached_rdma_channel_head = 0, global_rdma_tail_idx = 0;
        auto send_buffer = lane_id == rdma_rank ? rdma_channel_data.recv_buffer(lane_id) : rdma_channel_data.send_buffer(lane_id);
        auto local_lb_workspace = lb_workspace_for(nvl_rank);
        int source_flow_ordinal = 0;
        int rail_route_ordinals[NUM_MAX_NVL_PEERS] = {};
        const int lb_epoch =
            kLoadBalance ? *local_lb_workspace.epoch : 0;
        if constexpr (kLoadBalance) {
            while (ld_acquire_sys_global(
                       local_lb_workspace.schedule_ready
                   ) != lb_epoch) {
                __nanosleep(NUM_WAIT_NANOSECONDS);
            }
            if (lane_id < kNumRDMARanks and channel_id > 0) {
                source_flow_ordinal =
                    local_lb_workspace.source_channel_prefix[
                        local_lb_workspace.source_channel_index(
                            lane_id, channel_id - 1, num_channels
                        )
                    ];
            }
        }
        for (token_idx = token_start_idx; token_idx < token_end_idx; ++ token_idx) {
            ...
            NvlRankMask is_token_in_rank_mask = 0;
            uint8_t lb_destination_mask = 0;
            uint8_t lb_selected_rail = rail_lb::kInvalidRail;
            int lb_route_ordinal = -1;
            if (lane_id < kNumRDMARanks) {
                if constexpr (kLoadBalance) {
                    const auto original_mask = __ldg(
                        reinterpret_cast<const NvlRankMask*>(
                            is_token_in_rank +
                            token_idx * num_ranks +
                    ...
                    if (original_mask != 0) {
                        lb_selected_rail = rail_lb::select_rail(
                            local_lb_workspace.quota_row(lane_id),
                            nvl_rank,
                            source_flow_ordinal++
                        );
                        EP_DEVICE_ASSERT(
                            lb_selected_rail < NUM_MAX_NVL_PEERS
                        );
                        lb_route_ordinal =
                            rail_route_ordinals[
                                lb_selected_rail
                        ...
                        for (int destination_rail = 0;
                            ...
                            lb_destination_mask |=
                                static_cast<uint8_t>(
                                    values[destination_rail]
                                ) << destination_rail;
                        }
                        if (lb_selected_rail == nvl_rank)
                            is_token_in_rank_mask = original_mask;
                    }
                } else {
                    is_token_in_rank_mask = __ldg(
                        reinterpret_cast<const NvlRankMask*>(
                            is_token_in_rank +
                            token_idx * num_ranks +
                            lane_id * NUM_MAX_NVL_PEERS
            ...
            for (int i = 0, slot_idx; i < kNumRDMARanks; ++ i) if ((slot_idx = __shfl_sync(0xffffffff, rdma_tail_idx, i)) >= 0) {
                ...
                auto recv_is_token_in_rank_values = reinterpret_cast<const bool*>(&recv_is_token_in_rank_mask);
            ...
            if constexpr (kLoadBalance) {
                // Pack only surplus routes into the selected Rail's existing
                // IPC workspace. This runs inside the original V1 dispatch
                // kernel, so LB does not add a separate packing launch.
                int stage_slot = -1;
                int repair_slot = -1;
                uint8_t* stage_buffer = nullptr;
                int* stage_ready = nullptr;
                if (lane_id < kNumRDMARanks and
                    lb_destination_mask != 0 and
                    lb_selected_rail != nvl_rank) {
                    const int route_index =
                        local_lb_workspace.route_index(
                            lb_selected_rail, lane_id, channel_id,
                            kNumRDMARanks, num_channels
                    );
                    stage_slot =
                        local_lb_workspace.stage_offsets[route_index] +
                        lb_route_ordinal;
                    repair_slot =
                        local_lb_workspace.repair_offsets[route_index] +
                        lb_route_ordinal;
                    auto selected_workspace =
                        lb_workspace_for(lb_selected_rail);
                    EP_DEVICE_ASSERT(
                        stage_slot >= 0 and
                        static_cast<size_t>(stage_slot) <
                            selected_workspace.payload_capacity
                    );
                    stage_buffer = selected_workspace.payload +
                        static_cast<size_t>(stage_slot) *
                        lb_record_stride;
                    stage_ready =
                        selected_workspace.stage_ready +
                        stage_slot;
                    if constexpr (not kCachedMode) {
                        selected_workspace.virtual_token_indices[
                            stage_slot
                        ] = static_cast<int>(token_idx);
                        selected_workspace.virtual_source_rails[
                            stage_slot
                        ] = static_cast<uint8_t>(nvl_rank);
                        selected_workspace.virtual_channels[
                            stage_slot
                        ] = static_cast<uint8_t>(channel_id);
                        selected_workspace.virtual_repair_slots[
                            stage_slot
                        ] = repair_slot;
                        local_lb_workspace.repair_next[repair_slot] =
                            atomicExch(
                                local_lb_workspace.repair_heads +
                                    token_idx,
                                repair_slot
                            );
                    }
                ...
                int num_stage_routes = 0;
                void* stage_buffers[kNumTopkRDMARanks];
                int* stage_ready_ptrs[kNumTopkRDMARanks];
                SourceMeta stage_src_meta;
                #pragma unroll
                for (int destination_server = 0;
                     destination_server < kNumRDMARanks;
                     ++destination_server) {
                    const int peer_slot = __shfl_sync(
                        0xffffffff, stage_slot, destination_server
                    );
                    if (peer_slot < 0)
                        continue;
                    const auto peer_mask = static_cast<uint8_t>(
                        __shfl_sync(
                            0xffffffff,
                            static_cast<int>(lb_destination_mask),
                            destination_server
                        )
                    );
                    NvlRankMask expanded_mask = 0;
                    auto values =
                        reinterpret_cast<uint8_t*>(&expanded_mask);
                    #pragma unroll
                    for (int destination_rail = 0;
                         destination_rail < NUM_MAX_NVL_PEERS;
                         ++destination_rail) {
                        values[destination_rail] =
                            (peer_mask >> destination_rail) & 1u;
                    }
                    if (lane_id == num_stage_routes) {
                        stage_src_meta = SourceMeta(
                            rdma_rank,
                            reinterpret_cast<const bool*>(
                                &expanded_mask
                            )
                    ...
                    stage_buffers[num_stage_routes] =
                        broadcast(stage_buffer, destination_server);
                    stage_ready_ptrs[num_stage_routes] =
                        broadcast(stage_ready, destination_server);
                    ++num_stage_routes;
                }
                EP_DEVICE_ASSERT(
                    num_stage_routes <= kNumTopkRDMARanks
                );

                auto stage_broadcast =
                    [=](const int key, const int4& value) {
                        #pragma unroll
                        for (int output = 0;
                             output < num_stage_routes;
                             ++output) {
                            st_na_global(
                                reinterpret_cast<int4*>(
                                    stage_buffers[output]
                                ) + key,
                                value
                            );
                        }
                ...
                UNROLLED_WARP_COPY(
                    5, lane_id, hidden_int4, 0,
                    x + token_idx * hidden_int4,
                    ld_nc_global, stage_broadcast
                );
                #pragma unroll
                for (int output = 0;
                     output < num_stage_routes;
                ...
                #pragma unroll
                for (int scale = lane_id;
                     scale < num_scales;
                     scale += 32) {
                    const int offset =
                        token_idx * scale_token_stride +
                        scale * scale_hidden_stride;
                    const auto value =
                        ld_nc_global(x_scales + offset);
                    #pragma unroll
                    for (int output = 0;
                         output < num_stage_routes;
                         ++output) {
                ...
                #pragma unroll
                for (int index = lane_id;
                     index < num_topk * num_stage_routes;
                     index += 32) {
                    const int output = index / num_topk;
                    const int topk = index % num_topk;
                    const auto idx_value = static_cast<int>(
                        ld_nc_global(
                            topk_idx +
                            token_idx * num_topk + topk
                        )
                    );
                    const auto weight_value = ld_nc_global(
                        topk_weights +
                        token_idx * num_topk + topk
                    );
                    st_na_global(
        ...
        if constexpr (kLoadBalance) {
            // Feed surplus messages into the same V1 ring and completion
            // window, preserving chunking and backpressure.
            int tails[kNumRDMARanks];
            #pragma unroll
            for (int destination_server = 0;
                 destination_server < kNumRDMARanks;
                 ++destination_server) {
                tails[destination_server] =
                    local_lb_workspace.route_counts[
                        local_lb_workspace.route_index(
                            nvl_rank, destination_server, channel_id,
                            kNumRDMARanks, num_channels
                        )
                    ];
            }
            ...
            for (int producer = 0;
                ...
                for (int destination_server = 0;
                    ...
                    const int count =
                        producer_workspace.route_counts[route_index];
                    const int stage_begin =
                        producer_workspace.stage_offsets[route_index];
                    for (int ordinal = 0; ordinal < count; ++ordinal) {
                        const int assigned_warp =
                            work_index++ % kNumDispatchRDMASenderWarps;
                        if (assigned_warp != warp_id)
                            continue;

                        const int stage_slot = stage_begin + ordinal;
                        auto start_time = clock64();
                        while (ld_acquire_sys_global(
                                   local_lb_workspace.stage_ready +
                                   stage_slot
                               ) != lb_epoch) {
                            if (clock64() - start_time >=
                                NUM_TIMEOUT_CYCLES) {
                                printf(
                                    "DeepEP-LB source staging timeout, "
                                    "channel: %d, RDMA: %d, nvl: %d, "
                                    "producer: %d, slot: %d\n",
                        ...
                        const int rdma_tail_idx =
                            tails[destination_server] + ordinal;

                        start_time = clock64();
                        while (rdma_tail_idx -
                               static_cast<int>(ld_volatile_global(
                                   rdma_channel_head.buffer(
                                       destination_server
                                   )
                            ...
                            if (clock64() - start_time >=
                                NUM_TIMEOUT_CYCLES) {
                                printf(
                                    "DeepEP-LB staged sender timeout, "
                                    "channel: %d, RDMA: %d, nvl: %d, "
                                    "dst RDMA: %d, tail: %d\n",
                                    channel_id, rdma_rank, nvl_rank,
                                    destination_server, rdma_tail_idx
                                );
                                trap();
                            }
                        }

                        auto source = local_lb_workspace.payload +
                            static_cast<size_t>(stage_slot) *
                            lb_record_stride;
                        auto destination_buffer =
                            destination_server == rdma_rank
                                ? rdma_channel_data.recv_buffer(
                                    destination_server
                                  )
                                : rdma_channel_data.send_buffer(
                                    destination_server
                                  );
                        auto destination = destination_buffer +
                            static_cast<size_t>(
                                rdma_tail_idx %
                                num_max_rdma_chunked_recv_tokens
                            ) * num_bytes_per_token;
                        for (int offset = lane_id * sizeof(int4);
                             offset < num_bytes_per_token;
                             offset += 32 * sizeof(int4)) {
                            st_na_global(
                                reinterpret_cast<int4*>(
                                    destination + offset
                                ),
                        ...
                        if (lane_id == 0) {
                            const int virtual_row =
                                num_tokens + stage_slot;
                            if constexpr (not kCachedMode) {
                                send_rdma_head[
                                    virtual_row * kNumRDMARanks +
                                    destination_server
                                ] = rdma_tail_idx;
                            }
                    ...
                    tails[destination_server] += count;
                }
            }
        }
...
#define DISPATCH_LAUNCH_CASE(num_rdma_ranks) { \
    auto dispatch_func = low_latency_mode ? \
        (is_cached_dispatch ? \
             (load_balance ? dispatch<true, num_rdma_ranks, true, true, kNumTMABytesPerWarp, kNumDispatchRDMASenderWarps> : \
                             dispatch<true, num_rdma_ranks, true, false, kNumTMABytesPerWarp, kNumDispatchRDMASenderWarps>) : \
         load_balance ? dispatch<true, num_rdma_ranks, false, true, kNumTMABytesPerWarp, kNumDispatchRDMASenderWarps> : \
                        dispatch<true, num_rdma_ranks, false, false, kNumTMABytesPerWarp, kNumDispatchRDMASenderWarps>) : \
        (is_cached_dispatch ? \
             (load_balance ? dispatch<false, num_rdma_ranks, true, true, kNumTMABytesPerWarp, kNumDispatchRDMASenderWarps> : \
                             dispatch<false, num_rdma_ranks, true, false, kNumTMABytesPerWarp, kNumDispatchRDMASenderWarps>) : \
         load_balance ? dispatch<false, num_rdma_ranks, false, true, kNumTMABytesPerWarp, kNumDispatchRDMASenderWarps> : \
    ...
    LAUNCH_KERNEL(&cfg, dispatch_func, \
                  ...
                  lb_workspace_offset, lb_max_tokens, lb_record_stride, \
...
template <bool kLowLatencyMode, int kNumTMABytesPerWarp, bool kLoadBalance>
__global__ void cached_notify(const int rdma_clean_offset, const int rdma_num_int_clean,
    ...
    if (sm_id == 0) {
        ...
        if (is_cached_dispatch or kLoadBalance)
            return;
...
void cached_notify(int hidden_int4, int num_scales, int num_topk_idx, int num_topk_weights,
                   ...
                   bool is_cached_dispatch, bool load_balance,
                   bool low_latency_mode) {
    ...
    auto cached_notify_func = low_latency_mode
        ? (load_balance
            ? cached_notify<true, kNumTMABytesPerWarp, true>
            : cached_notify<true, kNumTMABytesPerWarp, false>)
        : (load_balance
            ? cached_notify<false, kNumTMABytesPerWarp, true>
            : cached_notify<false, kNumTMABytesPerWarp, false>);
...
__device__ int combine_token(bool is_token_in_rank, int head_idx,
    ...
    return num_topk_ranks > 0 ? topk_ranks[0] : -1;
}

template<bool kLowLatencyMode,
         int kNumRDMARanks, bool kLoadBalance, typename dtype_t,
...
combine(int4* combined_x, float* combined_topk_weights,
        ...
        const int* virtual_token_indices,
        const uint8_t* virtual_source_rails,
        const uint8_t* virtual_channels,
        int64_t lb_workspace_offset, int lb_max_tokens,
        size_t lb_record_stride,
        void* rdma_buffer_ptr, int num_max_rdma_chunked_send_tokens, int num_max_rdma_chunked_recv_tokens,
        void** buffer_ptrs, int num_max_nvl_chunked_send_tokens, int num_max_nvl_chunked_recv_tokens,
    ...
    if (warp_role == WarpRole::kNVLSender) {
        ...
        if (warp_role == WarpRole::kNVLAndRDMAForwarder) {
            ...
            // Fixed-Rail V1 partitions the original token rows directly.
            // DLB appends moved-message rows; their channel is recorded by the
            // fused source-side staging path.
            int token_start_idx = 0;
            int token_end_idx = num_combined_tokens;
            if constexpr (not kLoadBalance) {
                get_channel_task_range(
                    num_combined_tokens, num_channels, channel_id,
                    token_start_idx, token_end_idx
                );
            }
            ...
            const int task_begin = token_start_idx;
            const int task_end = token_end_idx;
            for (int64_t token_idx = task_begin + warp_id;
                 token_idx < task_end;
                ...
                if constexpr (kLoadBalance) {
                    ...
                    const int num_original_tokens =
                        *local_workspace.num_tokens;
                    if (token_idx < num_original_tokens) {
                        int direct_begin = 0, direct_end = 0;
                        get_channel_task_range(
                            num_original_tokens, num_channels, channel_id,
                            direct_begin, direct_end
                        );
                        if (token_idx < direct_begin or
                            token_idx >= direct_end) {
                            continue;
                        }
                    } else {
                        moved_index =
                            static_cast<int>(token_idx) -
                            num_original_tokens;
                        if (virtual_channels[moved_index] != channel_id)
                            continue;
                    }
                }
                // Read expected head
                ...
                const bool is_moved =
                    kLoadBalance and moved_index >= 0;
                const int original_token =
                    is_moved
                        ? virtual_token_indices[moved_index]
                        : static_cast<int>(token_idx);
                const int original_rail =
                    is_moved
                        ? virtual_source_rails[moved_index]
                        : nvl_rank;
                const bool apply_bias = not is_moved;
                int repair_slot = 0;
                int* repair_ready = nullptr;
                int4* combined_row =
                    combined_x + original_token * hidden_int4;
                float* combined_topk_row =
                    combined_topk_weights == nullptr
                        ? nullptr
                        : combined_topk_weights +
                            original_token * num_topk;
                if (is_moved) {
                    auto origin_workspace =
                        lb_workspace_for(original_rail);
                    repair_slot =
                        lb_workspace_for(nvl_rank)
                            .virtual_repair_slots[moved_index];
                    EP_DEVICE_ASSERT(
                        repair_slot >= 0 and
                        static_cast<size_t>(repair_slot) <
                            origin_workspace.payload_capacity
                    );
                    auto repair_payload = origin_workspace.payload +
                        static_cast<size_t>(repair_slot) *
                        lb_record_stride;
                    combined_row =
                        reinterpret_cast<int4*>(repair_payload);
                    combined_topk_row = num_topk == 0
                        ? nullptr
                        : reinterpret_cast<float*>(
                            repair_payload + hidden_bytes
                        );
                    repair_ready =
                        origin_workspace.repair_ready +
                        repair_slot;
                }
                combine_token<kNumRDMARanks, true, dtype_t, kNumTopkRDMARanks, false, 2>(
                    ...
                    combined_row,
                    combined_topk_row,
                    bias_0 == nullptr or not apply_bias
                        ? nullptr
                        : bias_0 + original_token * hidden_int4,
                    bias_1 == nullptr or not apply_bias
                        ? nullptr
                        : bias_1 + original_token * hidden_int4,
                    num_max_rdma_chunked_recv_tokens,
                ...
                if constexpr (kLoadBalance) {
                    if (lane_id == 0) {
                        ...
                        if (is_moved) {
                            st_release_sys_global(
                                repair_ready,
                                *lb_workspace_for(original_rail).epoch
                            );
                        } else {
                            auto local_workspace =
                                lb_workspace_for(nvl_rank);
                            st_release_sys_global(
                                local_workspace.direct_ready +
                                    original_token,
                                *local_workspace.epoch
                            );
                        }
                    }
                }
            ...
            if constexpr (kLoadBalance) {
                // Every channel first produces all partial results above.
                // Then the origin Rail consumes peer partials in the same
                // fused combine kernel, avoiding standalone repair launches.
                auto local_workspace = lb_workspace_for(nvl_rank);
                const int num_original_tokens =
                    *local_workspace.num_tokens;
                const int epoch = *local_workspace.epoch;
                int channel_begin = 0, channel_end = 0;
                get_channel_task_range(
                    num_original_tokens, num_channels, channel_id,
                    channel_begin, channel_end
                );
                for (int token = warp_id;
                     token < num_original_tokens;
                     token += kNumRDMAReceivers) {
                    if (token < channel_begin or token >= channel_end)
                        continue;
                    int repair_slot =
                        local_workspace.repair_heads[token];
                    if (repair_slot < 0)
                        continue;

                    auto start_time = clock64();
                    while (ld_acquire_sys_global(
                               local_workspace.direct_ready + token
                           ) != epoch) {
                        if (clock64() - start_time >
                            NUM_TIMEOUT_CYCLES) {
                            printf(
                                "DeepEP-LB combine direct-result timeout, "
                                "channel: %d, RDMA: %d, nvl: %d, "
                                "token: %d\n",
                                channel_id, rdma_rank, nvl_rank, token
                            );
                            trap();
                        }
                        __nanosleep(NUM_WAIT_NANOSECONDS);
                    ...
                    while (repair_slot >= 0) {
                        ...
                        start_time = clock64();
                        while (ld_acquire_sys_global(
                                   local_workspace.repair_ready +
                                   repair_slot
                               ) != epoch) {
                            if (clock64() - start_time >
                                NUM_TIMEOUT_CYCLES) {
                                printf(
                                    "DeepEP-LB combine repair timeout, "
                                    "channel: %d, RDMA: %d, nvl: %d, "
                                    "repair slot: %d, token: %d\n",
                                    channel_id, rdma_rank, nvl_rank,
                                    repair_slot, token
                                );
                                trap();
                        ...
                        const auto repair_payload =
                            local_workspace.payload +
                            static_cast<size_t>(repair_slot) *
                            lb_record_stride;
                        for (int index = lane_id;
                             index < hidden_int4;
                             index += 32) {
                            auto output = reinterpret_cast<dtype_t*>(
                                combined_x +
                                token * hidden_int4 + index
                            );
                            const auto partial =
                                reinterpret_cast<const dtype_t*>(
                                    repair_payload +
                                    index * sizeof(int4)
                                );
                            constexpr int kValuesPerInt4 =
                                sizeof(int4) / sizeof(dtype_t);
                            #pragma unroll
                            for (int element = 0;
                                 element < kValuesPerInt4;
                                 ++element) {
                                output[element] =
                                    static_cast<dtype_t>(
                                        static_cast<float>(
                        ...
                        repair_slot =
                            local_workspace.repair_next[repair_slot];
                    }
                    __syncwarp();
                }
            }
...
#define COMBINE_LAUNCH_CASE(num_rdma_ranks) { \
    auto combine_func = low_latency_mode ? \
        (load_balance ? \
            combine<true, num_rdma_ranks, true, nv_bfloat16, kNumCombineForwarderWarps, kNumTMABytesPerSenderWarp, kNumTMABytesPerForwarderWarp> : \
            combine<true, num_rdma_ranks, false, nv_bfloat16, kNumCombineForwarderWarps, kNumTMABytesPerSenderWarp, kNumTMABytesPerForwarderWarp>) : \
        (load_balance ? \
            combine<false, num_rdma_ranks, true, nv_bfloat16, kNumCombineForwarderWarps, kNumTMABytesPerSenderWarp, kNumTMABytesPerForwarderWarp> : \
            combine<false, num_rdma_ranks, false, nv_bfloat16, kNumCombineForwarderWarps, kNumTMABytesPerSenderWarp, kNumTMABytesPerForwarderWarp>); \
    ...
    LAUNCH_KERNEL(&cfg, combine_func, \
                  ...
                  virtual_token_indices, virtual_source_rails, virtual_channels, \
                  lb_workspace_offset, lb_max_tokens, lb_record_stride, \
                  rdma_buffer_ptr, num_max_rdma_chunked_send_tokens, num_max_rdma_chunked_recv_tokens, \
                  buffer_ptrs, num_max_nvl_chunked_send_tokens, num_max_nvl_chunked_recv_tokens, \
                  rank, num_ranks); } break
