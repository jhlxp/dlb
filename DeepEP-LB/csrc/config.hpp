...
struct Config {
    ...
    size_t get_lb_buffer_size_hint(int max_tokens, int hidden,
                                   int num_topk, int num_ranks) const {
        EP_HOST_ASSERT(max_tokens > 0 and hidden > 0 and num_topk > 0);
        EP_HOST_ASSERT(num_ranks > NUM_MAX_NVL_PEERS and
                       num_ranks % NUM_MAX_NVL_PEERS == 0);
        const int num_servers = num_ranks / NUM_MAX_NVL_PEERS;
        constexpr int kNumMaxLBChannels = 32;
        const auto record_stride = rail_lb::get_record_stride(
            hidden, num_topk, internode::get_source_meta_bytes()
        );
        return rail_lb::get_workspace_size(
            max_tokens, num_servers, kNumMaxLBChannels, record_stride
        );
    }
};
