...
struct Buffer {
    EP_STATIC_ASSERT(NUM_MAX_NVL_PEERS == 4 or NUM_MAX_NVL_PEERS == 8,
                     "The number of NVLink peers per logical server must be 4 or 8");
...
private:
    ...
    int64_t num_lb_bytes;
    int lb_max_tokens;
    int lb_round = 0;
    size_t lb_record_stride = 0;
    ...
    // Number of compact virtual source rows owned by the selected Rail.
    volatile int* lb_virtual_count = nullptr;
    int* lb_virtual_count_mapped = nullptr;
    volatile int* lb_outgoing_count = nullptr;
    int* lb_outgoing_count_mapped = nullptr;
