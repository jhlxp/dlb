...
Buffer::Buffer(int rank, int num_ranks, int64_t num_nvl_bytes, int64_t num_rdma_bytes,
               ...
               int64_t num_lb_bytes, int lb_max_tokens):
        rank(rank), num_ranks(num_ranks),
        num_nvl_bytes(num_nvl_bytes), num_lb_bytes(num_lb_bytes),
        lb_max_tokens(lb_max_tokens), num_rdma_bytes(num_rdma_bytes),
    ...
    EP_HOST_ASSERT(num_nvl_bytes % NUM_BUFFER_ALIGNMENT_BYTES == 0 and
                   num_lb_bytes % NUM_BUFFER_ALIGNMENT_BYTES == 0 and
                   (num_nvl_bytes <= std::numeric_limits<int>::max() or num_rdma_bytes == 0));
    EP_HOST_ASSERT((num_lb_bytes == 0) == (lb_max_tokens == 0));
    ...
    if (num_nvl_bytes > 0) {
        ...
        const auto num_local_bytes = num_nvl_bytes + num_lb_bytes;
        CUDA_CHECK(cudaMalloc(&buffer_ptrs[nvl_rank], num_local_bytes + barrier_signal_bytes + buffer_ptr_bytes + barrier_signal_ptr_bytes));
        CUDA_CHECK(cudaIpcGetMemHandle(&ipc_handles[nvl_rank], buffer_ptrs[nvl_rank]));
        buffer_ptrs_gpu = reinterpret_cast<void**>(static_cast<uint8_t*>(buffer_ptrs[nvl_rank]) + num_local_bytes + barrier_signal_bytes);

        // Set barrier signals
        barrier_signal_ptrs[nvl_rank] = reinterpret_cast<int*>(static_cast<uint8_t*>(buffer_ptrs[nvl_rank]) + num_local_bytes);
        barrier_signal_ptrs_gpu = reinterpret_cast<int**>(static_cast<uint8_t*>(buffer_ptrs[nvl_rank]) + num_local_bytes + barrier_signal_bytes + buffer_ptr_bytes);
    ...
    *moe_recv_counter = -1;
    ...
    if (num_lb_bytes > 0) {
        CUDA_CHECK(cudaMallocHost(
            &lb_virtual_count, sizeof(int), cudaHostAllocMapped
        ));
        CUDA_CHECK(cudaHostGetDevicePointer(
            &lb_virtual_count_mapped,
            const_cast<int*>(lb_virtual_count), 0
        ));
        CUDA_CHECK(cudaMallocHost(
            &lb_outgoing_count, sizeof(int), cudaHostAllocMapped
        ));
        CUDA_CHECK(cudaHostGetDevicePointer(
            &lb_outgoing_count_mapped,
            const_cast<int*>(lb_outgoing_count), 0
        ));
        *lb_virtual_count = -1;
        *lb_outgoing_count = -1;
    }
...
void Buffer::destroy() {
    ...
    if (lb_virtual_count != nullptr)
        CUDA_CHECK(cudaFreeHost(const_cast<int*>(lb_virtual_count)));
    if (lb_outgoing_count != nullptr)
        CUDA_CHECK(cudaFreeHost(const_cast<int*>(lb_outgoing_count)));
        ...
        for (int i = 0, offset = rdma_rank * num_nvl_ranks; i < num_nvl_ranks; ++ i) {
            ...
            if (offset + i != rank) {
                ...
                barrier_signal_ptrs[i] = reinterpret_cast<int*>(
                    static_cast<uint8_t*>(buffer_ptrs[i]) +
                    num_nvl_bytes + num_lb_bytes
                );
            } else {
...
Buffer::internode_dispatch(const torch::Tensor& x, const std::optional<torch::Tensor>& x_scales,
                           ...
                           int cached_num_recv_tokens, int cached_num_rdma_recv_tokens, int cached_num_virtual_tokens,
                           ...
                           const std::optional<torch::Tensor>& cached_lb_handle,
                           int expert_alignment, bool load_balance, const Config& config,
                           std::optional<EventHandle>& previous_event, bool async, bool allocate_on_comm_stream) {
#ifndef DISABLE_NVSHMEM
    ...
    if (cached_mode) {
        ...
        EP_HOST_ASSERT(load_balance == cached_lb_handle.has_value());
    } else {
        EP_HOST_ASSERT(num_tokens_per_rank.has_value());
        EP_HOST_ASSERT(num_tokens_per_rdma_rank.has_value());
        EP_HOST_ASSERT(num_tokens_per_expert.has_value());
        EP_HOST_ASSERT(not cached_lb_handle.has_value());
    }
    ...
    if (cached_mode) {
        ...
        if (cached_lb_handle.has_value())
            EP_HOST_ASSERT(cached_lb_handle->scalar_type() == torch::kUInt8);
    } else {
        EP_HOST_ASSERT(num_tokens_per_rank->scalar_type() == torch::kInt32);
    ...
    if (cached_mode) {
        ...
        if (cached_lb_handle.has_value())
            EP_HOST_ASSERT(cached_lb_handle->dim() == 1 and cached_lb_handle->is_contiguous());
    } else {
        EP_HOST_ASSERT(num_tokens_per_rank->dim() == 1 and num_tokens_per_rank->is_contiguous());
    ...
    auto lb_handle = std::optional<torch::Tensor>();
    int lb_num_incoming = 0;
    int lb_num_outgoing = 0;
    int lb_round_id = -1;
    if (load_balance) {
        EP_HOST_ASSERT(num_lb_bytes > 0 and lb_max_tokens >= num_tokens);
        EP_HOST_ASSERT(num_rdma_ranks > 1);
        EP_HOST_ASSERT(num_channels <= rail_lb::kMaxChannels);
        auto local_lb_workspace =
            static_cast<uint8_t*>(buffer_ptrs[nvl_rank]) +
            num_nvl_bytes;

        if (cached_mode) {
            EP_HOST_ASSERT(lb_record_stride > 0);
            EP_HOST_ASSERT(cached_num_virtual_tokens >= num_tokens);
            lb_num_incoming =
                cached_num_virtual_tokens - num_tokens;
            lb_num_outgoing = rail_lb::get_handle_outgoing_count(
                cached_lb_handle->numel(),
                num_tokens,
                lb_num_incoming,
                num_rdma_ranks,
                num_channels
            ...
            EP_HOST_ASSERT(
                cached_lb_handle->numel() ==
                static_cast<int64_t>(rail_lb::get_handle_size(
                    num_tokens,
                    lb_num_incoming,
                    lb_num_outgoing,
                    num_rdma_ranks,
            ...
            intranode::barrier(
                barrier_signal_ptrs_gpu, nvl_rank,
                num_nvl_ranks, comm_stream
            );
        } else {
            lb_record_stride = rail_lb::get_record_stride(
                hidden, num_topk, internode::get_source_meta_bytes()
            );
            EP_HOST_ASSERT(
                rail_lb::get_workspace_size(
                    lb_max_tokens, num_rdma_ranks,
                    num_channels, lb_record_stride
                ) <= static_cast<size_t>(num_lb_bytes)
            );

            *lb_virtual_count = -1;
            *lb_outgoing_count = -1;
            lb_round_id = lb_round++;
        }
    ...
    if (cached_mode) {
        ...
        internode::cached_notify(hidden_int4, num_scales, num_topk, num_topk,
                                 ...
                                 num_nvl_bytes, true, load_balance,
                                 low_latency_mode);
    } else {
        ...
        *moe_recv_counter = -1, *moe_recv_rdma_counter = -1;
        ...
        internode::notify_dispatch(num_tokens_per_rank->data_ptr<int>(), moe_recv_counter_mapped, num_ranks,
                                   ...
                                   load_balance, num_nvl_bytes,
                                   lb_max_tokens, lb_record_stride,
                                   lb_round_id,
                                   load_balance
                                       ? lb_virtual_count_mapped
                                       : nullptr,
    ...
    const int num_virtual_tokens = load_balance
        ? (cached_mode
               ? cached_num_virtual_tokens
               : static_cast<int>(*lb_virtual_count))
        : num_tokens;
    EP_HOST_ASSERT(num_virtual_tokens >= 0);
    if (load_balance and not cached_mode) {
        lb_num_incoming = num_virtual_tokens - num_tokens;
        lb_num_outgoing = static_cast<int>(*lb_outgoing_count);
        EP_HOST_ASSERT(lb_num_incoming >= 0 and lb_num_outgoing >= 0);
        const auto handle_bytes = rail_lb::get_handle_size(
            num_tokens,
            lb_num_incoming,
            lb_num_outgoing,
            num_rdma_ranks,
        ...
        lb_handle = torch::empty(
            {static_cast<int64_t>(handle_bytes)},
    ...
    int* recv_rdma_channel_prefix_ptr = nullptr;
    int* recv_gbl_channel_prefix_ptr = nullptr;
    ...
    if (not cached_mode) {
        ...
        send_rdma_head = load_balance
            ? torch::full(
                {num_virtual_tokens, num_rdma_ranks}, -1,
                dtype(torch::kInt32).device(torch::kCUDA)
              )
            : torch::empty(
                {num_tokens, num_rdma_ranks},
                dtype(torch::kInt32).device(torch::kCUDA)
              );
        ...
        recv_rdma_channel_prefix_ptr =
            recv_rdma_channel_prefix_matrix->data_ptr<int>();
        recv_gbl_channel_prefix_ptr =
    ...
    if (load_balance and not cached_mode) {
        auto local_lb_workspace =
            static_cast<uint8_t*>(buffer_ptrs[nvl_rank]) +
            num_nvl_bytes;
        EP_HOST_ASSERT(lb_handle.has_value());
        rail_lb::save_handle(
            lb_handle->data_ptr(),
            local_lb_workspace,
            num_tokens,
    ...
    if (async) {
        ...
        for (auto& to: {x_scales, topk_idx, topk_weights,
                        ...
                        recv_src_meta, lb_handle, cached_lb_handle}) {
            to.has_value() ? to->record_stream(comm_stream) : void();
            if (allocate_on_comm_stream)
                to.has_value() ? to->record_stream(compute_stream) : void();
        }
    ...
    return {recv_x, recv_x_scales, recv_topk_idx, recv_topk_weights, num_recv_tokens_per_expert_list,
            ...
            recv_src_meta, send_rdma_head, send_nvl_head, lb_handle, event};
#else
    EP_HOST_ASSERT(false and "NVSHMEM is disabled during compilation");
    return {};
#endif
...
Buffer::internode_combine(const torch::Tensor& x, const std::optional<torch::Tensor>& topk_weights,
                          ...
                          const std::optional<torch::Tensor>& lb_handle,
                          bool load_balance, const Config& config,
                          std::optional<EventHandle>& previous_event, bool async, bool allocate_on_comm_stream) {
#ifndef DISABLE_NVSHMEM
    const int num_channels = config.num_sms / 2;
    EP_HOST_ASSERT(config.num_sms % 2 == 0);
    ...
    EP_HOST_ASSERT(load_balance == lb_handle.has_value());
    if (lb_handle.has_value())
        EP_HOST_ASSERT(lb_handle->dim() == 1 and lb_handle->is_contiguous() and
                       lb_handle->scalar_type() == torch::kUInt8);

    auto num_tokens = static_cast<int>(x.size(0)), hidden = static_cast<int>(x.size(1)), hidden_int4 = static_cast<int>(x.size(1) * x.element_size() / sizeof(int4));
    const auto num_original_tokens = static_cast<int>(
        is_combined_token_in_rank.size(0)
    );
    const auto num_combined_tokens = load_balance
    ...
    int* lb_virtual_token_indices = nullptr;
    uint8_t* lb_virtual_source_rails = nullptr;
    uint8_t* lb_virtual_channels = nullptr;
    if (load_balance) {
        EP_HOST_ASSERT(num_lb_bytes > 0 and
                       lb_max_tokens >= num_original_tokens);
        EP_HOST_ASSERT(lb_record_stride > 0);
        auto local_lb_workspace =
            static_cast<uint8_t*>(buffer_ptrs[nvl_rank]) +
            num_nvl_bytes;
        const int num_incoming =
            num_combined_tokens - num_original_tokens;
        EP_HOST_ASSERT(num_incoming >= 0);
        const int num_outgoing =
            rail_lb::get_handle_outgoing_count(
                lb_handle->numel(),
                num_original_tokens,
                num_incoming,
                num_rdma_ranks,
        ...
        const rail_lb::Workspace lb_workspace(
            local_lb_workspace, lb_max_tokens,
            num_rdma_ranks, num_channels,
            lb_record_stride
        );
        lb_virtual_token_indices = lb_workspace.virtual_token_indices;
        lb_virtual_source_rails = lb_workspace.virtual_source_rails;
        lb_virtual_channels = lb_workspace.virtual_channels;
        rail_lb::finalize_combined_heads(
            combined_rdma_head.data_ptr<int>(),
            local_lb_workspace,
            num_combined_tokens, lb_max_tokens,
            num_rdma_ranks, num_channels,
            lb_record_stride, comm_stream
        );
    }
    ...
    internode::cached_notify(hidden_int4, 0, 0, num_topk,
                             ...
                             barrier_signal_ptrs_gpu, rank, comm_stream,
                             config.get_rdma_buffer_size_hint(hidden_int4 * sizeof(int4), num_ranks),
                             num_nvl_bytes, false, load_balance,
                             low_latency_mode);
    ...
    for (int i = 0; i < 2; ++ i) if (bias_opts[i].has_value()) {
        ...
        EP_HOST_ASSERT(bias.scalar_type() == x.scalar_type());
        EP_HOST_ASSERT(
            bias.size(0) == num_original_tokens and
            bias.size(1) == hidden
        );
        bias_ptrs[i] = bias.data_ptr();
    }
    ...
    auto combined_x = torch::empty(
        {
            load_balance ? num_original_tokens : num_combined_tokens,
            hidden
        },
        x.options()
    );
    internode::combine(at::cuda::ScalarTypeToCudaDataType(x.scalar_type()),
                       ...
                       load_balance,
                       lb_virtual_token_indices,
                       lb_virtual_source_rails,
                       lb_virtual_channels,
                       num_nvl_bytes, lb_max_tokens,
                       lb_record_stride,
                       rdma_buffer_ptr, config.num_max_rdma_chunked_send_tokens, config.num_max_rdma_chunked_recv_tokens,
                       buffer_ptrs_gpu, config.num_max_nvl_chunked_send_tokens, config.num_max_nvl_chunked_recv_tokens,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    pybind11::class_<deep_ep::Config>(m, "Config")
        ...
        .def("get_lb_buffer_size_hint", &deep_ep::Config::get_lb_buffer_size_hint);
    ...
    m.def("get_num_nvl_peers", deep_ep::get_num_nvl_peers);
}
