...
class Buffer:
    ...
    def __init__(self, group: Optional[dist.ProcessGroup],
        ...
        Arguments:
            ...
            use_lb: enable source-server-local DLB Rail balancing.
            lb_max_tokens: maximum number of local tokens accepted by an LB dispatch.
            lb_hidden: hidden dimension used by LB dispatch.
            lb_num_topk: maximum TopK width used by LB dispatch.
        """
        ...
        self.use_lb = bool(use_lb)
        self.lb_max_tokens = int(lb_max_tokens)
        self.lb_hidden = int(lb_hidden)
        self.lb_num_topk = int(lb_num_topk)
        num_lb_bytes = 0
        if self.use_lb:
            num_nvl_peers = deep_ep_cpp.get_num_nvl_peers()
            if low_latency_mode:
                raise ValueError('DLB is supported only by high-throughput internode dispatch')
            if self.group_size <= num_nvl_peers:
                raise ValueError('DLB requires at least two servers')
            if self.group_size % num_nvl_peers != 0:
                raise ValueError('EP size must be divisible by GPUs per server')
            if min(lb_max_tokens, lb_hidden, lb_num_topk) <= 0:
                raise ValueError(
                    'lb_max_tokens, lb_hidden, and lb_num_topk must be positive '
                    'when use_lb=True'
                )
            config = self.get_dispatch_config(self.group_size)
            num_lb_bytes = config.get_lb_buffer_size_hint(
                lb_max_tokens, lb_hidden, lb_num_topk, self.group_size
            )
        elif any(value != 0 for value in (lb_max_tokens, lb_hidden, lb_num_topk)):
            raise ValueError('LB sizing arguments require use_lb=True')

        self.runtime = deep_ep_cpp.Buffer(
            self.rank, self.group_size,
            num_nvl_bytes, num_rdma_bytes,
            low_latency_mode, explicitly_destroy,
            num_lb_bytes, lb_max_tokens,
        )
    ...
    def internode_dispatch(self, x: Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]],
        ...
        x, x_scales = x if isinstance(x, tuple) else (x, None)
        if self.use_lb:
            if x.size(0) > self.lb_max_tokens:
                raise ValueError('LB dispatch exceeds lb_max_tokens')
            if x.size(1) != self.lb_hidden:
                raise ValueError('LB dispatch hidden size differs from lb_hidden')
            if handle is None and (
                topk_idx is None or topk_idx.size(1) != self.lb_num_topk
            ):
                raise ValueError('LB dispatch TopK width differs from lb_num_topk')
        if handle is not None:
            assert topk_idx is None and topk_weights is None
            if self.use_lb:
                is_token_in_rank, \
                    ...
                    recv_src_meta, send_rdma_head, send_nvl_head, lb_handle = handle
            else:
                is_token_in_rank, \
                    rdma_channel_prefix_matrix, gbl_channel_prefix_matrix, \
                    recv_rdma_channel_prefix_matrix, recv_rdma_rank_prefix_sum, recv_gbl_channel_prefix_matrix, recv_gbl_rank_prefix_sum, \
            ...
            num_virtual_tokens = send_rdma_head.size(0) if self.use_lb else 0
            recv_x, recv_x_scales, _, _, _, _, _, _, _, _, _, _, _, _, _, event = self.runtime.internode_dispatch(
                x, x_scales, topk_idx, topk_weights,
                None, None, is_token_in_rank, None,
                num_recv_tokens, num_rdma_recv_tokens, num_virtual_tokens,
                rdma_channel_prefix_matrix, recv_rdma_rank_prefix_sum, gbl_channel_prefix_matrix, recv_gbl_rank_prefix_sum,
                lb_handle,
                expert_alignment, self.use_lb, config,
            ...
            recv_x, recv_x_scales, recv_topk_idx, recv_topk_weights, num_recv_tokens_per_expert_list, \
                ...
                recv_src_meta, send_rdma_head, send_nvl_head, lb_handle, event = self.runtime.internode_dispatch(
                x, x_scales, topk_idx, topk_weights,
            ...
            if self.use_lb:
                assert lb_handle is not None
                handle = (*handle, lb_handle)
            return (recv_x, recv_x_scales) if x_scales is not None else recv_x, recv_topk_idx, recv_topk_weights, num_recv_tokens_per_expert_list, handle, EventOverlap(event)
    ...
    def internode_combine(self, x: torch.Tensor, handle: Union[tuple, list],
        ...
        if self.use_lb:
            is_combined_token_in_rank, \
                ...
                src_meta, send_rdma_head, send_nvl_head, lb_handle = handle
        else:
            is_combined_token_in_rank, \
                _, _, \
                rdma_channel_prefix_matrix, rdma_rank_prefix_sum, gbl_channel_prefix_matrix, gbl_rank_prefix_sum, \
        ...
        combined_x, combined_topk_weights, event = self.runtime.internode_combine(
            ...
            send_rdma_head, send_nvl_head, lb_handle, self.use_lb, config,
            getattr(previous_event, 'event', None),
            async_finish, allocate_on_comm_stream)
